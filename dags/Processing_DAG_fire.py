...
def fetch_repo_id(**kwargs):
    ...
    system_params_dict = kwargs['ti'].xcom_pull(task_ids='get_system_params')
    ...
    logging.info(f"Fetching tags and dataform workflow IDs from query: {query}")

    try:
        rows = pandas_gbq.read_gbq(query, project_id=project_id)
        logging.info(f"Query results: {rows.to_string(index=False)}")
        if rows.empty:
        ...
        logging.info(f'Repo ID: {repo_id}') 
        logging.info(f'Data Form Tags: {dataform_tags}')
        logging.info(f'Data Form Execution ID: {dataform_execution_id}')
        repo_id = {"repo_id": repo_id, "dataform_tags": dataform_tags, "dataform_execution_id": dataform_execution_id}
        logging.info(f'Repo Info: {repo_id}') 
        kwargs['ti'].xcom_push(key='repo_info', value=repo_id)
        return repo_id
...
def check_if_dataform_job_running(**kwargs):
    ...
    project_id = kwargs['ti'].xcom_pull(task_ids='get_gcp_project_id', key='project_id')
    logging.info(f"Checking running Dataform jobs in project: {project_id}")

    # Get repository ID and Dataform tags from XCom
    fetch_repo_id = kwargs['ti'].xcom_pull(task_ids='fetch_repo_id', key='repo_info')
    repository_id = fetch_repo_id['repo_id']
    logging.info(f"Repository ID: {repository_id}")
    dataform_tags = fetch_repo_id['dataform_tags']
    logging.info(f"Checking for active jobs with any of these tags: {dataform_tags}")
    dataform_execution_id = fetch_repo_id['dataform_execution_id']
    if dataform_execution_id is None:
        logging.info(f"data form execution id is not found for tag {dataform_tags}")
        return
    logging.info(f"Dataform tags: {dataform_tags}")
    # Initialize Dataform API client
    ...
    try:
        ...
        if state in ('RUNNING'):
            logging.error(f"{dataform_tags} is already running")
            raise Exception("Dataform already running")
        else:
            logging.info(f"{dataform_tags} is not running")
    except Exception as e:
        logging.error(f"Error retrieving workflow invocation {dataform_tags}: {e}")
...
def create_compilation_result(**kwargs):
    ...
    logging.info(f"Dataform default branch: {repository_branch}")
    fetch_repo_id = ti.xcom_pull(task_ids='fetch_repo_id')
    repository_id = fetch_repo_id['repo_id']
    logging.info(f'repository id: {repository_id}')
    logging.info("Compilation Results starting...")
    try:
        ...
        logging.info("Compilation Results obtained...")
        ti.xcom_push(key='compilation_result_name', value=result['name'])
    except HttpError as http_err:
        logging.error(f"HTTP error during compilation: {http_err}")
        raise Exception(f"HTTP error during compilation: {http_err}")
    except AirflowException as ae:
        logging.error(f"Airflow operator failure during compilation: {ae}")
        raise Exception(f"Airflow error during compilation: {ae}")
    except Exception as e:
        logging.error(f"Unexpected error in create_compilation_result(): {e}")
...
def create_workflow_invocation(**kwargs):
    ...
    try:
        ...
        logging.info(f"Dataform workflow invoked with name: {invocation_name}")

        workflow_invocation_id = invocation_name.split('/')[-1]
        logging.info(f"Workflow Invocation ID: {workflow_invocation_id}")

        kwargs['ti'].xcom_push(key='workflow_invocation_id', value=workflow_invocation_id)

        logging.info("Dataform workflow Invocated...")
    except HttpError as http_err:
        logging.error(f"HTTP error during workflow invocation: {http_err}")
        raise Exception(f"HTTP error during workflow invocation: {http_err}")
    except AirflowException as ae:
        logging.error(f"Airflow operator failure during workflow invocation: {ae}")
        raise Exception(f"Airflow error during workflow invocation: {ae}")
    except Exception as e:
        logging.error(f"Unexpected error in create_workflow_invocation(): {e}")
